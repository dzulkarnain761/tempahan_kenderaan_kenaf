...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $cara_deposit = 'atas talian';

    // Prepare and execute the update for tempahan status
    $sql1 = $conn->prepare("UPDATE tempahan SET status_tempahan = ?, status_bayaran = ?, cara_deposit = ? WHERE tempahan_id = ?");
    $sql1->bind_param("sssi", $status, $status, $cara_deposit, $id);
    ...
    $conn->close();
    echo json_encode(['success' => true, 'message' => 'Deposit Berjaya Dibayar']);
}
?>
